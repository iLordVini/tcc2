...
TOKEN = "wYf774fyGotOY30u0yHQnzb7p6VYsT-t5UCRE97W"
HINT = "nocodb"
MONITOR = str((Path(__file__).parent.parent / "monitor_ram.py").resolve())

TABLE_PRODUCTS = "mnjffmzxhy9jt2t"
  ...
  if (rand < 0.80) {{
    ...
    group('POST - Criar', function () {{
      const create = http.post(
        `${{BASE_URL}}/tables/${{TABLE_PRODUCTS}}/records`,
        JSON.stringify({{ name: "Produto Teste NocoDB", price: 1 }}),
        {{ headers }}
      );
      check(create, {{ 'POST 200': (r) => r.status === 200 }});
      sleep(0.2);
    }});
  }}
}}
...
def run_k6(profile_name, workdir: Path, run_dir: Path):
    ...
    subprocess.run(
        ["python", MONITOR, "--hint", HINT, "--interval", "1", "--out-dir", str(run_dir), "--tag", tag, "--cmd", cmd],
        check=False,
    )
    res = {}
...
def main():
    ...
    resultados = {"ferramenta": "NocoDB", "cenario": "80/20", "timestamp": start_ts, "testes": {}}
    workdir = script_dir
    for profile_name in ["load", "stress", "spike", "soak"]:
        resultados["testes"][profile_name] = run_k6(profile_name, workdir, run_dir)
        time.sleep(3)
    (script_dir / f"results_nocodb_80_20_{start_ts}.json").write_text(
